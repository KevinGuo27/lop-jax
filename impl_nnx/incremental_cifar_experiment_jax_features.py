...
from modified_resnet_nnx_cbp_features import build_resnet18
...
class IncrementalCIFARExperiment(Experiment):
    ...
    def __init__(self, exp_params: dict, results_dir: str, run_index: int, verbose=True):
        ...
        self.agent_type = exp_params.get("agent_type", "base_dl")
        self.use_effective_rank = (self.agent_type == "effective_rank")

        self.agent = build_agent(
            kind = self.agent_type,
            shrink = exp_params.get("shrink_factor", 0.01),
            sigma = self.noise_std,
            resgnt = self.resgnt # for CBPAgent
        )
    ...
    def evaluate_network(self, test_data):
        ...
        """
        @nnx.jit
        def eval_step(model, images, labels, current_classes):
            logits_full, features = model(images)
    ...
    def train(self, train_dataloader : DataLoader, test_dataloader : DataLoader, val_dataloader : DataLoader,
          ...
          # 1. train step 
          # 2. eval step
          # 3. hessian computation (end of task)
          # 4. extend classes
          # 5. hessian computation (start of task)
        ...
        # compute Hessian at start of task 0
        ...
        for e in tqdm(range(self.current_epoch, self.num_epochs)):
            ...
            for step_number, sample in enumerate(train_dataloader):
                ...
                classes_tuple = tuple(self.all_classes[:self.current_num_classes])

                self.net.train() # this also updates the train_state model mode
            ...
            # Compute Hessian at start of new task 
