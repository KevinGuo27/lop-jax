...
def get_results_path(args, return_npy: bool = True):
    results_dir = Path('/users/tserapio/lop-jax/incremental_cifar/results')
...
def plot_hessian_spectrum(grids_train, density_train, grids_test, density_test, task_num, agent_name, at_init: bool = True, save_data: bool = True):
    ...
    out_dir = Path("/users/tserapio/lop-jax/incremental_cifar", "hessian", agent_name)
    ...
    if save_data:
        ...
        out_dir = Path("/users/tserapio/lop-jax/incremental_cifar", "hessian", "data", agent_name) 
        out_dir.mkdir(parents=True, exist_ok=True)
