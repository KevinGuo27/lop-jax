...
_TASK_RE = re.compile(r"hessian_task_(\d+)_([a-z_]+)\.pdf$")
...
def _extract_tasks(algorithm_dir: Path, stage: str) -> dict[int, Path]:
    ...
    for pdf in algorithm_dir.glob(f"hessian_task_*_{stage}.pdf"):
        m = _TASK_RE.match(pdf.name)
...
def _load_image(path: Path | None) -> Image.Image:
    """Return a Pillow image; if path is a PDF, convert its first page to an image."""
    if path and path.exists():
        suffix = path.suffix.lower()
        if suffix == ".pdf":
            # render first page of PDF at 200dpi
